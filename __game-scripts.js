...
Attack.prototype.initialize = function() {
    ...
    this.app.on('actionHandler->attack::attack', (data) => {
        ...
        if ( this.entity.name.substring(0, this.entity.name.length - 1 ) ==  "Mystic" ||
...
var FindModelNew=pc.createScript("findModelNew");FindModelNew.prototype.initialize=function(){this.app.mouse.on(pc.EVENT_MOUSEMOVE,this.onMouseMove,this),this.app.mouse.on(pc.EVENT_MOUSEDOWN,this.onMouseDown,this),this.mapSize=31,this.tileSize=4,this.screenWidth=this.app.graphicsDevice.width,this.screenHeight=this.app.graphicsDevice.height,this.minXScale=278/1441,this.maxXScale=1196/1441,this.minZScale=25/749,this.maxZScale=718/749,this.minX=this.minXScale*this.screenWidth,this.maxX=this.maxXScale*this.screenWidth,this.minZ=this.minZScale*this.screenHeight,this.maxZ=this.maxZScale*this.screenHeight;var i=-449.0187500397762,t=445.5581110238632,s=-47.47281684444641,e=-79.78259060746717,h=s-i,a=e-t,n=526.5287606902953,o=-402.377234996554;this.firstSize=Math.sqrt(h*h+a*a),this.secondSize=Math.sqrt(n*n+o*o),this.firstM=a/h,this.secondM=o/n,this.firstC=this.firstM*i-t,this.secondC=this.secondM*s-e,this.firstScale=692.5/575.1,this.secondScale=692.5/575.1},FindModelNew.prototype.update=function(i){},FindModelNew.prototype.onMouseMove=function(i){this.findModelInWorld(i)},FindModelNew.prototype.onMouseDown=function(i){var t=Math.PI/180,s=new pc.Vec3,e=this.app.root.findByName("Camera");e.camera.screenToWorld(i.x,i.y,10,s);var h=s.x,a=s.y,n=s.z,o=e.getEulerAngles().x*t,c=e.getEulerAngles().y*t,r=e.getEulerAngles().z*t,M=Math.cos(o)*Math.sin(c)*Math.cos(r)+Math.sin(o)*Math.sin(r),d=Math.cos(o)*Math.sin(c)*Math.sin(r)-Math.sin(o)*Math.cos(r),p=h-a*M/d,l=n-a*(Math.cos(o)*Math.cos(c))/d;if(l-this.firstM*p+this.firstC>=0&&l-this.secondM*p+this.secondC>=0){var m=Math.abs(l-this.firstM*p+this.firstC)/286.168806036706*this.firstSize,f=Math.abs(l-this.secondM*p+this.secondC)/218.5628759023298*this.secondSize,S=Math.floor(m/this.firstSize*this.mapSize),z=Math.floor(f/this.secondSize*this.mapSize),g=this.app.root.findByName("Map");if(z>=0&&z<this.mapSize&&S>=0&&S<this.mapSize){var y=new pc.Entity;y=g.children[S*this.mapSize+z],this.entity.enabled&&(y.name=this.entity.name,y.model.asset=this.entity.model.asset)}}},FindModelNew.prototype.findModelInWorld=function(i){var t=Math.PI/180,s=new pc.Vec3,e=this.app.root.findByName("Camera");e.camera.screenToWorld(i.x,i.y,10,s);var h=s.x,a=s.y,n=s.z,o=e.getEulerAngles().x*t,c=e.getEulerAngles().y*t,r=e.getEulerAngles().z*t,M=Math.cos(o)*Math.sin(c)*Math.cos(r)+Math.sin(o)*Math.sin(r),d=Math.cos(o)*Math.sin(c)*Math.sin(r)-Math.sin(o)*Math.cos(r),p=h-a*M/d,l=n-a*(Math.cos(o)*Math.cos(c))/d;if(l-this.firstM*p+this.firstC>=0&&l-this.secondM*p+this.secondC>=0){var m=Math.abs(l-this.firstM*p+this.firstC)/286.168806036706*this.firstSize,f=Math.abs(l-this.secondM*p+this.secondC)/218.5628759023298*this.secondSize,S=Math.floor(m/this.firstSize*this.mapSize),z=Math.floor(f/this.secondSize*this.mapSize),g=this.app.root.findByName("Map");z>=0&&z<this.mapSize&&S>=0&&S<this.mapSize&&this.entity.setPosition(g.children[S*this.mapSize+z].getPosition().x,.1,g.children[S*this.mapSize+z].getPosition().z)}};